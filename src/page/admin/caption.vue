  ...
  <div class="container m-auto px-4 duration-300">
    ...
    <div class="mt-5">
      <div class="mt-3 grid gap-4 md:grid-cols-2 xl:grid-cols-3">
        <div v-for="node in list.content" class="p-4 bg-white rounded-md border border-zinc-300 dark:bg-zinc-900/50 dark:border-zinc-800 shadow-sm">
          ...
          <div class="mt-4 flex">
            <input type="datetime-local" v-model="node.updDt" pattern="[0-9]{4}-[0-9]{2}-[0-9]{2}T[0-9]{2}:[0-9]{2}" class="text-center outline-none bg-gray-50 border border-gray-300 text-gray-900 sm:text-sm rounded-md focus:border-blue-600 block w-full p-2.5 dark:bg-zinc-900 dark:border-zinc-800 dark:placeholder-gray-400 dark:text-white dark:focus:border-zinc-300" />
            <input type="button" value="현재" @click="setNow(node)" class="w-[60px] ml-2 text-gray-700 dark:text-white bg-zinc-100 hover:bg-zinc-200 outline-none font-medium rounded-md text-sm py-2.5 text-center dark:bg-zinc-800 dark:hover:bg-zinc-800/80" />
          </div>
          <div class="mt-4 flex">
            <input type="text" name="website" v-model="node.website" placeholder="https://example.com" class="outline-none bg-gray-50 border border-gray-300 text-gray-900 sm:text-sm rounded-md focus:border-blue-600 block w-full p-2.5 dark:bg-zinc-900 dark:border-zinc-800 dark:placeholder-gray-400 dark:text-white dark:focus:border-zinc-300">
            <input type="button" value="저장" @click="doSave(node)" class="w-[60px] ml-2 text-blue-800 dark:text-white bg-blue-200 hover:bg-blue-300 outline-none font-medium rounded-md text-sm py-2.5 text-center dark:bg-gray-800 dark:hover:bg-gray-800/80" />
          </div>
