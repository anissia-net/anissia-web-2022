<template>
  ...
  <div class="container m-auto px-4 duration-300">
    ...
    <div class="flex justify-between space-x-2 sm:space-x-3 md:space-x-4">
      <div v-for="(week, idx) in weekList" @click="getAnimeList(idx)" class="flex-auto text-center font-bold text-md md:text-lg py-2 block bg-white border border-zinc-200 rounded-md shadow-sm hover:bg-blue-50 dark:bg-zinc-900/60 dark:border-zinc-800/70 dark:hover:bg-zinc-900 cursor-pointer" :class="({'text-gray-400 dark:text-gray-600': idx != weekNow})">{{week}}</div>
    </div>

    <div class="mt-4">
      <div class="mt-3 grid gap-4 md:grid-cols-2 xl:grid-cols-3">
        <div v-for="(node, i) in animeList" class="p-4 bg-white rounded-md border border-zinc-300 dark:bg-zinc-900/50 dark:border-zinc-800 shadow-sm">
          <div>
